...
module Wescontrol
  class Wescontrol
    ...
    def start
      ...
      EventMachine::run {
        # Start router that manages pub/sub
        PubSubRouter.new(PUB_PATH, SUB_PATH).run(ctx)

        ReqRepRouter.new(DEVICE_REP_PATH, DEVICE_REQ_PATH).run(ctx)
